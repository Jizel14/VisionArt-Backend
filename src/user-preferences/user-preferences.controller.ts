...
import { AiAudioService } from '../ai-audio/ai-audio.service';
...
export class UserPreferencesController {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Generate a 3-track AI playlist from saved user preferences',
  })
  @ApiResponse({
    status: 201,
    description: 'AI playlist generated successfully',
    schema: {
      example: {
        tracks: [
          '/audio/file/generated-1.mp3',
          '/audio/file/generated-2.mp3',
          '/audio/file/generated-3.mp3',
        ],
      },
    },
  })
  async generatePlaylist(@CurrentUser() userId: string) {
    const preferences = await this.userPreferencesService.findByUserId(userId);
    const tracks = await this.aiAudioService.generatePlaylistFromPreferences(
      preferences.favoriteStyles ?? [],
      preferences.favoriteColors ?? [],
      preferences.preferredMood ?? null,
    );
    return { tracks };
  }
