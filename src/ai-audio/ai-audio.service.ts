...
export class AiAudioService {
  ...
  // Force Beatoven-only flow as requested.
  private readonly useUdioapi = false;
  ...
  async generatePlaylistFromPreferences(
    ...
    try {
      ...
      for (let i = 0; i < trackCount; i++) {
        ...
        try {
          ...
          if (e.message.includes('402')) {
            ...
            // Keep UX working by filling remaining slots with local fallback audio.
            for (let j = i; j < trackCount; j++) {
              const fallbackPrompt = `${basePrompt} (Fallback ${j + 1})`;
              tracks.push(this.createLocalFallbackTrack(fallbackPrompt, j));
            }
            break;
          }
        }
      ...
      if (tracks.length < trackCount) {
        this.logger.warn(
          `AI audio generation returned ${tracks.length}/${trackCount} tracks. Completing with local fallback audio.`,
        );
        for (let i = tracks.length; i < trackCount; i++) {
          tracks.push(this.createLocalFallbackTrack(`${basePrompt} (Auto Fallback ${i + 1})`, i));
        }
      }
  ...
  private createLocalFallbackTrack(prompt: string, index: number): string {
    const tempDir = path.resolve(process.cwd(), 'temp');
    if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir, { recursive: true });

    const fileName = `demo_playlist_${index + 1}_${randomUUID()}.wav`;
    const outputPath = path.resolve(tempDir, fileName);
    const buffer = this.createToneWavBuffer(index);
    fs.writeFileSync(outputPath, buffer);
    this.logger.warn(
      `Using local fallback audio for "${prompt}" because no audio provider returned track ${index + 1}.`,
    );
    return `/audio/file/${fileName}`;
  }

  private createToneWavBuffer(index: number): Buffer {
    ...
    for (let i = 0; i < samples; i++) {
      ...
      const envelope = Math.min(1, t / 0.3) * Math.min(1, (durationSeconds - t) / 0.8);
      const wave =
        Math.sin(2 * Math.PI * root * t) * 0.45 +
        Math.sin(2 * Math.PI * fifth * t) * 0.25 +
        Math.sin(2 * Math.PI * octave * t) * 0.15;
      const value = Math.max(-1, Math.min(1, wave * envelope * 0.35));
